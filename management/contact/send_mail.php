<?php
	...
	$subject = "Feature Request by $name: " . $subjects;
	$requests = $_REQUEST['requests'] ;
	
	// If the user tries to access this script directly, redirect them to feedback form,
	if (!isset($_REQUEST['email_address'])) {
	header( "Location: ../index.html?path=contact/request.html" );
	}
	
	// If the form fields are empty, redirect to the error page.
	elseif (empty($email_address) || empty($requests)) {
	header( "Location: ../index.html?path=contact/errormessage.html" );
	}
	...
	mail( "admin@ccanb.ca", $subject, $requests, "From: $email_address" );
	header( "Location: ../index.html?path=contact/thankyou.html" );
	}
?>